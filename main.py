...
def fetch_registros() -> list[dict[str, Any]]:
    if not supabase:
        return []

    response = (
        supabase.table("registros_nascentes")
        .select("*")
...
def listar_registros() -> dict[str, Any]:
    if not supabase:
        return {"error": "Supabase não configurado.", "registros": []}

    return {"registros": fetch_registros()}


@app.get("/registros")
def listar_registros_compat() -> dict[str, Any]:
    if not supabase:
        return {"error": "Supabase não configurado.", "registros": []}
